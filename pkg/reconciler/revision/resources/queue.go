...
func getResources(annotations map[string]string, userContainer *corev1.Container) corev1.ResourceRequirements {
	...
	if ok, resourcePercentage = getResourcePercentageFromAnnotations(annotations, serving.QueueSideCarResourcePercentageAnnotation); ok {
		...
		if ok, requestCPU = getUserContainerResourceRequirements(userContainer.Resources.Requests.Cpu(), resourcePercentage); ok {
			resourceRequests[corev1.ResourceCPU] = requestCPU
		}

		if ok, limitCPU = getUserContainerResourceRequirements(userContainer.Resources.Limits.Cpu(), resourcePercentage); ok {
			resourceLimits[corev1.ResourceCPU] = limitCPU
		}

		if ok, requestMemory = getUserContainerResourceRequirements(userContainer.Resources.Requests.Memory(), resourcePercentage); ok {
			resourceRequests[corev1.ResourceMemory] = requestMemory
		}

		if ok, limitMemory = getUserContainerResourceRequirements(userContainer.Resources.Limits.Memory(), resourcePercentage); ok {
...
func getUserContainerResourceRequirements(resourceQuantity *resource.Quantity, percentage float32) (bool, resource.Quantity) {
	if !resourceQuantity.IsZero() {
		...
		//return true, resource.MustParse(resourceQuantity.AsDec().Mul(percentage).String())
	}
